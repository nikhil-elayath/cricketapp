...
describe("testing player api's", () => {
  ...
  it("should return a status code of 200, the body should be an object, return a response message, return no of times a batsman has scored 50s and 100s data and should be an array", done => {
    request(app)
      .get("/apis/PlayerInfo/BatsmanStats/" + 48)
      .then(res => {
        expect(res.body.status).toBe(200);
        expect(res.body).toEqual(expect.any(Object));
        expect(res.body.message).toBe("no. of times 50s and 100s retrieved");
        done();
      });
  });

  it("should return a status code of 400, the body should be an object, return an error message ", done => {
    request(app)
      .get("/apis/PlayerInfo/BatsmanStats/" + 4800)
      .then(res => {
        expect(res.body.statusCode).toBe(400);
        // expect(res.body).toEqual(expect.any(Object));
        expect(res.body.statusMessage).toBe(

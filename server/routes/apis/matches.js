...
router.get('/summary/:id', async (req, res, next) => {
    try {
        ...
        // date of the match
        const dbdates = await db.any(`select match_date from match_date where match_id=${id}`);
        console.log(dbdates)
        ...
        // teamone name
        const teamone_name = await db.any(`select team_name as teamone_name from team where team_id=${match_details[0].innings_one_team}`)
        ...
        // teamtwo name
        const teamtwo_name = await db.any(`select team_name as teamtwo_name from team where team_id=${match_details[0].innings_two_team}`)
